...
This patch runs after the text stability guard and directly fixes three tested
areas: QTextEdit undo/delete keys, saving active Text Box content before canvas
copy/cut, and preserving Text Box attributes during clone/paste.
"""
...
PATCH_VERSION = "engineering-text-box-copy-key-fix-2026-07-02-a"
...
def _save_editor(editor: QTextEdit | None) -> None:
    ...
    try:
        ...
        logging.exception("text_box_copy_key_fix: save editor failed")
...
def _patch_canvas_text_editor() -> None:
    try:
        ...
        return
    if getattr(editor_cls, "_text_box_copy_key_fix", "") == PATCH_VERSION:
        return
    ...
    editor_cls._text_box_copy_key_fix = PATCH_VERSION
...
def _patch_app_text_filter() -> None:
    try:
        ...
        return
    if getattr(filter_cls, "_text_box_copy_key_fix", "") == PATCH_VERSION:
        return
    ...
    filter_cls._text_box_copy_key_fix = PATCH_VERSION


def _patch_clone(edw) -> None:
    cls = edw.CanvasObject
    if getattr(cls, "_text_box_copy_key_fix_clone", "") == PATCH_VERSION:
        return
    old_clone = cls.clone
    ...
    cls.clone = clone
    cls._text_box_copy_key_fix_clone = PATCH_VERSION


def _patch_canvas_copy(edw) -> None:
    canvas_cls = edw.EngineeringCanvas
    if getattr(canvas_cls, "_text_box_copy_key_fix_copy", "") == PATCH_VERSION:
        return
    ...
    canvas_cls._text_box_copy_key_fix_copy = PATCH_VERSION
...
def apply_text_box_copy_key_fix_patch() -> None:
    try:
        ...
        logging.exception("text_box_copy_key_fix: workspace import failed")
        return
    _patch_canvas_text_editor()
    _patch_app_text_filter()
    _patch_clone(edw)
    _patch_canvas_copy(edw)
    logging.info("text_box_copy_key_fix: installed version=%s", PATCH_VERSION)
