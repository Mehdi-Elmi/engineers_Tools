...
from PySide6.QtGui import QColor, QKeySequence, QTextBlockFormat
from PySide6.QtWidgets import QButtonGroup, QColorDialog, QHBoxLayout, QPushButton, QTextEdit, QWidget

PATCH_VERSION = "engineering-text-toolbar-final-event-safety-2026-07-02-d"
...
def _patch_color_dialog(text_lag) -> None:
    ...
    def open_standard_color_dialog(parent: QWidget, current: str) -> str | None:
        try:
            from . import text_line_math_symbols_patch as line_patch
            dialog, body, body_layout = line_patch._dialog_shell(parent, "Add Custom Color", (560, 430))
            ...
            return picker.currentColor().name() if dialog.exec() == QDialog.DialogCode.Accepted and picker.currentColor().isValid() else None
        except Exception:
            return None
...
def apply_text_toolbar_final_event_safety_patch() -> None:
    ...
    def workspace_init(self, module) -> None:
        ...
        for delay in (0, 120, 400):
            QTimer.singleShot(delay, lambda root=self: (word._apply_word_toolbar(root), _wire_list_buttons(root)))
