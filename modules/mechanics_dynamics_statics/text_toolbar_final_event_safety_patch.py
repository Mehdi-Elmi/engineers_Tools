...
PATCH_VERSION = "engineering-text-toolbar-final-event-safety-2026-07-02-q-restore"
...
def _patch_canvas_clone(edw) -> None:
    item_cls = getattr(edw, "CanvasItem", None)
    if item_cls is None or getattr(item_cls, "_text_metadata_clone_patch", False):
        return
    old_clone = getattr(item_cls, "clone", None)
    if not callable(old_clone):
        return

    def clone(self, *args, **kwargs):
        new_item = old_clone(self, *args, **kwargs)
        _copy_extra_metadata(self, new_item)
        return new_item

    item_cls.clone = clone
    item_cls._text_metadata_clone_patch = True
...
def _patch_project_clipboard_metadata() -> None:
    try:
        ...
        return
    if getattr(project_io, "_text_metadata_clipboard_patch", False):
        return

    old_item_to_dict = getattr(project_io, "item_to_dict", None)
    old_item_from_dict = getattr(project_io, "item_from_dict", None)
    if not callable(old_item_to_dict) or not callable(old_item_from_dict):
        return

    def item_to_dict(item):
        data = old_item_to_dict(item)
        if isinstance(data, dict):
            for field in _TEXTBOX_EXTRA_FIELDS:
                if hasattr(item, field):
                    try:
                        data[field] = copy.deepcopy(getattr(item, field))
                    except Exception:
                        data[field] = getattr(item, field)
        return data

    def item_from_dict(data):
        item = old_item_from_dict(data)
        if isinstance(data, dict) and item is not None:
            for field in _TEXTBOX_EXTRA_FIELDS:
                if field in data:
                    try:
                        setattr(item, field, copy.deepcopy(data[field]))
                    except Exception:
                        setattr(item, field, data[field])
        return item

    project_io.item_to_dict = item_to_dict
    project_io.item_from_dict = item_from_dict
    project_io._text_metadata_clipboard_patch = True
